...
def display_row(image_path, range_text, category, description, background_color, padding_top_range, padding_top):
    ...
    col1, col2, col3 = st.columns([1.5, 1.7, 7.3])
    with col1:
        st.image(image_path, width=150)  
    with col2:
        st.markdown(f"<h3 style='padding-top: {padding_top_range}px; border-radius: 50px; text-align: center; background-color: {background_color}; height: 150px;'><strong>{range_text}</strong><div>{category}</div></h3>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<h3 style='border-radius: 50px; text-align: center; background-color: #3C88C9; height: 150px; padding-top: {padding_top}px'>{description}</h3>", unsafe_allow_html=True)
...
def health_advice_section(df):
    ...
    if current_aqi < 50:
        ...
        display_row("hazardous.png", "400 - 500","Hazardous", "Health warnings of emergency conditions. The entire population is likely to be affected. Stay indoors as much as possible. Use air purifiers and wear masks if you need to go outside. Follow government advisories closely.", "#994444", "30", "5")
    else:
