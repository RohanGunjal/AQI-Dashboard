...
st.markdown(f"""<h1 style='font-size: 5em';>Interactive <span style='font-size: 1em; color: green;'>Air Quality</span> Dashboard</h1>""", unsafe_allow_html=True)
st.write("---")
...
df_gangapur_forecasted['Timestamp'] = pd.to_datetime(df_gangapur_forecasted['Timestamp'], format='%Y-%m-%d %H:%M:%S', errors='coerce')

df_csmt_forecasted = pd.read_csv('CSMT_Forecasted_AQI.csv')
df_csmt_forecasted['Timestamp'] = pd.to_datetime(df_csmt_forecasted['Timestamp'], format='%Y-%m-%d %H:%M:%S', errors='coerce')
...
def GIS_integration(df1, df2, df1_f, df2_f, timestamp_method):
    ...
    if timestamp_method == "Last Available":
        ...
        if selected_map:
            ...
            folium_static(selected_map,width = 950)
        else:
        ...
        if current_map:
            ...
            folium_static(current_map, width=950)
        else:
...
def create_map(selected_time, data):
    ...
    if 'AQI' in filtered_data.columns:
        aqi_column = 'AQI'
    elif 'Forecasted_AQI' in filtered_data.columns:
        aqi_column = 'Forecasted_AQI'
    else:
...
def aqi_range():
    ...
    for i in range(num_columns):
        with row[i]:
            ...
            st.image(category['image_url'], width=190)
...
def forcasting():
    ...
    for i in range(current_day_index, current_day_index + 7):
        if i >= len(dfforecast):  # If index exceeds available data
            break
        aqi = dfforecast["Forecasted_AQI"][i]
...
def display_row(image_path, range_text, category, description, background_color, padding_top_range, padding_top):

    col1, col2, col3 = st.columns([0.35, 2, 7.5])         
    with col2:
        st.markdown(f"<h2 style='text-align: center; height: 100px; color:{background_color};'><strong>AQI Range</strong></h2>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<h2 style='text-align: center; height: 100px; color: #3C88C9;'><strong>Health Recommendations</strong></h2>", unsafe_allow_html=True)

    col1, col2, col3 = st.columns([1.5, 2, 9])
    with col1:
        st.image(image_path, width=150)  
    with col2:
        st.markdown(f"<h3 style='padding-top: {padding_top_range}px; border-radius: 50px; text-align: center; background-color: {background_color}; height: 150px;padding-left : 20px;padding-right :20px;'><strong>{range_text}</strong><div>{category}</div></h3>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<h3 style='border-radius: 50px; text-align: center; background-color: #3C88C9; height: 150px; padding-top: {padding_top}px;padding-left : 20px;padding-right :20px;'>{description}</h3>", unsafe_allow_html=True)

def health_advice_section(df):
    current_datetime = round_to_quarter_hour(datetime.now())
    df_filtered = df[df['Timestamp'] == current_datetime]
    #current_aqi = 125
    current_aqi = df_filtered['Forecasted_AQI'].iloc[0]

    st.title("Health Advices According to AQI")
    st.subheader(f"Current AQI Value is: {current_aqi}")
    st.write('\n')

    if current_aqi < 50:
        display_row("good.png", "0 - 50 ", "Good", "Air quality is considered satisfactory, and air pollution poses little or no risk. Enjoy outdoor activities and exercise freely.", "#76D250", "35", "35")
    elif current_aqi < 100:
        display_row("moderate.png", "50 - 100", "Moderate", "Air quality is acceptable however, there may be some pollutants that could pose a moderate health concern for a very small number of people. Sensitive individuals (such as those with respiratory or heart conditions) may want to limit prolonged outdoor exertion.", "#FFDF2F", "30", "10")
    elif current_aqi < 200:
        display_row("poor.png", "100 - 200", "Unhealthy for Sensitive Groups", "People with respiratory or heart conditions, children, and older adults may experience health effects. Reduce prolonged or heavy outdoor exertion, especially if you fall into any of the sensitive groups.", "#FB7C27", "10", "20")
    elif current_aqi < 300:
        display_row("unhealthy.png", "200 - 300" ,"Unhealthy", "Everyone may begin to experience adverse health effects, and members of sensitive groups may experience more serious effects. Avoid outdoor activities and minimize exposure to outdoor air. Stay indoors with air purifiers, if possible.", "#EA1B1B", "30", "10")
    elif current_aqi < 400:
        display_row("severe.png", "300 - 400","Very Unhealthy", "Health alert: everyone may experience more serious health effects. Avoid all outdoor activities, and remain indoors with air purifiers. Keep windows and doors closed.", "#B14691", "30", "20")
    elif current_aqi < 500:
        display_row("hazardous.png", "400 - 500","Hazardous", "Health warnings of emergency conditions. The entire population is likely to be affected. Stay indoors as much as possible. Use air purifiers and wear masks if you need to go outside. Follow government advisories closely.", "#994444", "30", "20")
    else:
        print("AQI data is not Available")
...
def historical_Analysis():
    ...
    st.subheader('Select Year for Historical Analysis')
...
def plot_air_pollution_data(df):
    ...
    colors = ['green' if aqi < 50 else 'lightgreen' if 50 <= aqi <= 100 else 'yellow' for aqi in df_filtered['Forecasted_AQI']]

    # Define marker sizes
    marker_sizes = [10 if i == df_filtered[df_filtered['Timestamp'] == current_datetime].index else 5 for i in range(len(df_filtered))]

    # Create a scatter plot
    fig = go.Figure(data=go.Scatter(x=df_filtered['Timestamp'], y=df_filtered['Forecasted_AQI'], mode='markers', marker=dict(color=colors, size=marker_sizes)))
    ...
    fig.update_yaxes(range=[0, max(df_filtered['Forecasted_AQI']) * 1.1])  # Adjust multiplier as needed
...
def main():
    ...
    health_advice_section(df_forecasted)
